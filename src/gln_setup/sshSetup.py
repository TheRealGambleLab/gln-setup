...
class SSHkey:
    ...
    def __post_init__(self):
        ...
        if self.name is None and self.key_path is not None:
    ...
    def create(self):
        if (
            ...
            raise ValueError(
                f"{self.key_path} aleady exists and force not set."
            )
        cmd = [
            "ssh-keygen",
            "-q",
    ...
    def send_to_server(self, target: str):
        ...
        if self.key_path is None:
            raise ValueError("key_path is not set")
        run(
            [
                "ssh-copy-id",
                "-i",
                str(self.key_path.with_suffix(".pub")),
                target,
            ],
            check=True,
        )
    ...
    def send_to_github(self):
        print(
            ...
            "to load your new ssh-key to github..."
        )
        run(
    ...
    def add_to_config(
        self,
        host: str,
        path: Path = Path("~/.ssh/config"),
        **hostOptions: dict[str, str],
    ):
        ...
        config = (
            read_ssh_config(path) if path.exists() else empty_ssh_config_file()
        )
        func = (
